...
export class StringUtil {
  ...
  public static kebabCase = (string: string) =>
    StringUtil.words(string.replace(/['\u2019]/g, '')).reduce(
      (result, word, index) => result + (index ? '-' : '') + word.toLowerCase(),
      '',
    );

  public static pascalCase = (string: string) =>
    StringUtil.words(string)
      .map((word) => word[0].toUpperCase() + word.slice(1).toLowerCase())
      .join('');

  public static words = (string: string, pattern?: RegExp | string) => {
  ...
  public static camelCase = (string: string) =>
    StringUtil.words(string.replace(/['\u2019]/g, '')).reduce((result, word, index) => {
      word = word.toLowerCase();
      return result + (index ? StringUtil.upperCaseFirstLetter(word) : word);
    }, '');
